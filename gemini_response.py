...
index = pc.Index("test-gemini-singlefile-docs")
...
def search_knowledge_base(query, top_k=15, alpha=0.5):
...
def get_answer(query, language):
    ...
    relevant_chunks = search_knowledge_base(query, top_k=15)
...
if __name__ == "__main__":
    ...
    result = get_consultation(
        text_input="পিপিআর সম্পর্কে বলুন এবং এটি কীভাবে ব্যবহার করবেন?",
        language="english"
    )
    print(f"\nAnswer:\n{result['answer']}")
    print(f"\nAnswer:\n{result['language']}")
