...
#         2. USE DOCUMENTS INTELLIGENTLY: Reference specific rules, but explain them in simple terms
#         3. PROVIDE SOLUTIONS: Don't just say "Rule X states Y" - explain HOW to apply it to their situation
...
#         ২. নথি বুদ্ধিমত্তার সাথে ব্যবহার করুন: নির্দিষ্ট নিয়ম উল্লেখ করুন, কিন্তু সহজ ভাষায় ব্যাখ্যা করুন
#         ৩. সমাধান প্রদান করুন: শুধু বলবেন না "নিয়ম X বলে Y" - তাদের পরিস্থিতিতে এটি কীভাবে প্রয়োগ করতে হবে তা ব্যাখ্যা করুন
...
#         "query": text_input,
#         "language": language,
#         "consultation": result['answer'],
#         "intent_analysis": result['intent_analysis'],
#         "sources": result['sources'],
#         "total_sources": result['total_sources_used']
#     }


# def main(text_input: str, language: str = "english") -> str:
#     """
#     Entrypoint for external callers.

#     Args:
#         text_input: User's question or problem description.
#         language: 'english' or 'bangla'.

#     Returns:
#         The consultation answer string (empty string if none).
#     """
#     result = get_consultation(text_input, language=language)
...
#     answer = main(language= "bangla", 
#                   text_input="সরকারি ক্রয়ে দরপত্র আহ্বান করার নিয়মাবলী কী?"
#                   )
...
def _hash_text(t: str):
    """Generate a deterministic hash for text (used in caching)."""
    return hashlib.sha256(t.encode("utf-8")).hexdigest()

# Simple in-memory cache for repeated embedding requests
@lru_cache(maxsize=512)
def cached_embedding(hash_key, text, openai_client):
...
class ProcurementConsultant:
    def __init__(self, index_name="ppr-documents"):
        ...
        3. PROVIDE SOLUTIONS: Don't just say "Rule X states Y" - explain HOW to apply it to their situation
        4. BE CONSULTATIVE: Offer alternatives, warn about common pitfalls, suggest best practices
        5. STAY ON TOPIC: Only handle Bangladesh public procurement matters
        6. BE PRACTICAL: If the documents don't cover something, use your general BPPA knowledge but indicate this clearly
        7. GUIDE STEP-BY-STEP: Break down complex processes into actionable steps

        RESPONSE STRUCTURE:
        - Acknowledge user's situation/problem
        - Provide relevant rule/regulation with clear explanation
        - Give practical, step-by-step guidance
        - Mention key considerations or warnings
        - Offer to clarify or help with next steps
    ...
    def get_embedding(self, text):
        ...
        h = _hash_text(text)
        return cached_embedding(h, text, self.openai_client)

    def analyze_query_intent(self, query, language="english"):
        """Analyze the user's query to identify their actual problem."""
        prompts = {
    ...
    def get_enhanced_context(self, query, top_k=3, snippet_chars=350):
        """Retrieve summarized context (short snippets) for efficiency."""
        results = self.search_documents(query, top_k=top_k)
        if not results:
            return "No relevant documents found."
        parts = []
        for match in results[:top_k]:
            snippet = shorten(match.metadata.get("text", ""), width=snippet_chars, placeholder="...")
            parts.append(f"[{match.metadata.get('source','Unknown')} | Page {match.metadata.get('page_number','N/A')}] {snippet}")
        return "\n\n".join(parts)

    def _choose_quotes_from_matches(self, matches, max_quotes=2, max_chars=300):
        """Select 1–2 short document excerpts for authenticity."""
        quotes = []
        for m in matches[:max_quotes]:
            txt = (m.metadata.get("text", "") or "").strip().replace("\n", " ")
            excerpt = shorten(txt, width=max_chars, placeholder="...")
            quotes.append({
                "quote": excerpt,
                "source": m.metadata.get("source", "Unknown"),
                "page": m.metadata.get("page_number", "N/A")
            })
        return quotes



    def consult(self, user_input, language="english", include_quotes_prob=0.5):
        """Provide expert consultation, optionally with quoted document excerpts."""
        if language not in ["english", "bangla"]:
            return "Error: Language must be 'english' or 'bangla'."

        # Analyze query & retrieve context
        intent = self.analyze_query_intent(user_input, language)
        results = self.search_documents(user_input, top_k=5)
        context = self.get_enhanced_context(user_input)

        # Choose system prompt
        system_prompt = (
            self.english_prompt if language == "english" else self.bangla_prompt
        ...
        try:
            ...
            # Randomly decide whether to include document quotes
            include_quotes = random.random() < include_quotes_prob
            quote_text = ""
            if include_quotes and results:
                quotes = self._choose_quotes_from_matches(results)
                quote_lines = [
                    f'> "{q["quote"]}" — {q["source"]} (Page {q["page"]})'
                    for q in quotes
                ]
                quote_text = "\n\n**Referenced excerpt(s):**\n" + "\n\n".join(quote_lines)

            # Add compact source list
            sources = [
                f"- {m.metadata.get('source','Unknown')} (Page {m.metadata.get('page_number','N/A')}) — preview: {shorten(m.metadata.get('text',''), width=100)}"
                for m in results[:5]
            ]
            source_block = "\n\n**Sources Used:**\n" + "\n".join(sources) if sources else ""

            return f"{answer_text}{quote_text}{source_block}"

        except Exception as e:
            error_msg = {
                "english": f"I apologize, but I encountered an error while analyzing your situation: {e}",
                "bangla": f"আমি দুঃখিত, আপনার পরিস্থিতি বিশ্লেষণ করার সময় একটি ত্রুটি ঘটেছে: {e}"
            }
            return error_msg[language]

    def consult_with_details(self, user_input, language="english"):
        """Return detailed consultation info (answer, intent, sources)."""
        intent = self.analyze_query_intent(user_input, language)
        results = self.search_documents(user_input, top_k=5)
        answer = self.consult(user_input, language)
        ...
        sources = [
            {
                "source": m.metadata.get("source", "Unknown"),
                "page": m.metadata.get("page_number", "N/A"),
                "relevance_score": round(getattr(m, "score", 0), 4),
                "language": m.metadata.get("language", "unknown"),
                "text_preview": (m.metadata.get("text", "") or "")[:300]
            }
            for m in results
        ]
        quotes = self._choose_quotes_from_matches(results)
...
def get_consultation(text_input, language="english"):
    """Main callable entry for external use."""
    consultant = ProcurementConsultant(index_name="bangladesh-procurement-docs")
    result = consultant.consult_with_details(text_input, language=language)
    return {
        # "query": text_input,
...
def main(text_input: str, language: str = "english") -> dict:
    """CLI entry point for testing."""
    return get_consultation(text_input, language=language)
