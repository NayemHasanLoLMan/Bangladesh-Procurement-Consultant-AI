...
if __name__ == "__main__":
    ...
    response1 = get_consultation(
        text_input="I need to procure IT equipment worth 50 lakh taka. What method should I use and what documents do I need?",
        language="english"
    )
    
    print(f"\nQuery: {response1['query']}")
    print(f"\nIntent: {response1['intent_analysis']}")
    print(f"\nConsultation:\n{response1['consultation']}")
    print(f"\nSources Used: {response1['total_sources']}")
    
    
    response2 = get_consultation(
        text_input="আমার টেন্ডার বাতিল হয়ে গেছে। আমি কী করতে পারি?",
        language="bangla"
    )
    
    print(f"\nপ্রশ্ন: {response2['query']}")
    print(f"\nপরামর্শ:\n{response2['consultation']}")
    
    
    consultant = ProcurementConsultant()
    answer = consultant.consult(
