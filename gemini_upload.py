...
BATCH_SIZE = 100
MAX_EMBEDDING_LENGTH = 2048  # Gemini's token limit for embeddings
...
class PDFUploader:
    ...
    def create_index(self):
        ...
        if self.index_name not in existing:
            ...
            self._wait_until_ready(timeout=25)
        self.index = self.pc.Index(self.index_name)

    def _wait_until_ready(self, timeout=25, poll=2.0):
        start = time.time()
        while True:
    ...
    def get_embedding(self, text):
        """Get embedding using Gemini with rate limiting"""
        max_retries = 3
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Add small delay to respect rate limits (15 RPM = 4 seconds between requests)
                time.sleep(4)
        ...
        for page_info in pages_data:
            ...
            for chunk_idx, chunk in enumerate(chunks):
                ...
                metadata = {
                    ...
                    "page": page_num,
                    "text": chunk[:3000]  # Store first 3000 chars for preview
                }
...
if __name__ == "__main__":
    uploader = PDFUploader(index_name="test-gemini-singlefile-docs")
